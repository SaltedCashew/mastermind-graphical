...
Pair Programming Assignment. You must work with one other person on this assignment using the pair programming technique. Review the paper on pair programming posted on the BB.   Your partner, and you will both get the same grade for the assignment.  Make sure both names are submitted with the design and code.  If you begin working with one partner and do not wish to finish the assignment with that partner then you must complete the assignment individually.  Your TA must be so informed before the due date. 

General Assignment Requirements

The purpose of this assignment is to design and implement an OO program with multiple classes to play a text based version of the board game called Mastermind. You are free to use whatever classes and methods from the JAVA library you wish to use. 
...
The top-level class of your program must be called Game.  It must have a constructor that takes a boolean value as its parameter. The boolean value is used for running the game in the testing mode until you are finished.  If it is true, then the secret code is revealed throughout the game. The Game class must also have a method named runGame that carries out the actual games. Your main method creates and uses a new Game object for each game played.

When finished turn in a file named assign2.zip that contains all the files needed for your program to be compiled and run. Include all the source code for all the classes you created.
...
Tip - Each team is required to use the git/bitbucket repository for interim and final versions of your code.  The rule of thumb is that you should commit at least once per working day when code is being generated; preferrably at the end of each working session where code is being changed.  Please see the git tutorial provided by your TA for more information on how to use that. 
